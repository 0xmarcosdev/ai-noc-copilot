...
## Sesión 21 — 3-4 sept 2026

**Asistente(s) de IA**: Grok (diagnóstico Docker, guía operativa, diseño TUI) · OpenCode + Nemotron 3 Ultra Free (aplicación en repo, hardening Docker, TUI fake logs)  
**Fase**: Fase 6 (Documentación y entrega) — **cierre MVP**

**Contexto**: Validación real de Opción B (`docker compose`) tras haber tenido solo inspección estática. Error de arranque del frontend en contenedor; necesidad de guía operativa reproducible y cierre de entrega sin grabación de vídeo (presentación = landing + Slidev + demo en vivo).

**Qué se hizo**:

*Con Grok (este chat):*

- Diagnosticado `ValueError: numpy.dtype size changed` al importar pandas en el contenedor frontend: `pandas==2.0.3` sin pin de numpy → resolución de pip a numpy 2.x incompatible con el ABI del wheel. Mitigación: `numpy==1.26.4` en `frontend/requirements.txt`.
- Aclarado flujo de logs sintéticos con app en Docker: `generate_fake_logs.py` corre en el **host** y envía UDP a `:5514` publicado por compose; Ollama sigue en el host vía `host.docker.internal`.
- Documentado prerrequisito `OLLAMA_HOST=0.0.0.0:11434` (persistente en Windows) y convivencia venv ↔ Docker sin rotar variables del backend/frontend a mano.
- Redactada guía operativa completa `docs/guia-docker-ai-noc.md` (arquitectura, prerrequisitos, deploy, debug, checklist de demo, errores frecuentes).
- Diseñado contrato de la TUI del generador de logs (escenario por número, count, 4 modos: enviar UDP / mostrar comando CLI / portapapeles / archivo para ingesta).

*Con OpenCode (Nemotron 3 Ultra Free):*

- Hardening del entregable Docker: imágenes **autocontenidas** (sin bind mounts de código en compose); `mkdir -p /app/data` en backend; `COPY static` + `--server.headless=true` en frontend; `.dockerignore` del frontend endurecido.
- Implementación de la TUI `scripts/fake_logs_tui.py` reutilizando la lógica de `generate_fake_logs.py` (CLI intacto).
- Cierre documental Fase 6: ROADMAP, DEVLOG, copia de la guía a `/docs`, alineación con presentación real (landing + Slidev + demo en vivo; **sin** grabación de vídeo como entregable).

**Decisión de diseño / aprendizaje clave**: El `docker-compose.yml` es el **entregable de despliegue** (imágenes inmutables + solo volumen de datos), no el entorno de desarrollo diario (sigue siendo venv). Iterar UI montando código del host es cómodo pero rompe portabilidad de la demo; se priorizó autocontención. El pin explícito de numpy evita un fallo silencioso de resolución de deps en builds limpios.

**Verificación**: `docker compose up -d --build` con backend healthy y frontend running; health API OK; logs sintéticos visibles al instante; llamada a Ollama desde el contenedor OK. Presentación ensayada (landing + Slidev + demo real). Tag previsto: **v1.0.0**.
